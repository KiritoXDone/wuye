      ...
      <div className="space-y-6 pb-2">
        ...
        <PageSection title="筛选条件" description="筛选并查询。">
          <div className="flex flex-wrap gap-3">
            ...
            <button type="button" className="btn-primary gap-2" onClick={() => void loadData(filters)} disabled={loading}><Search className="h-4 w-4" />查询</button>
        ...
        <PageSection title="日志列表" description="列表聚焦最关键的定位字段。">
          <AsyncState loading={loading} error={error} empty={!list.length} emptyDescription="当前条件下暂无审计日志。">
            ...
            <div className="mt-4 flex items-center justify-between text-sm text-slate-500"><span>共 {total} 条，当前第 {filters.pageNo} 页</span><div className="flex gap-2"><button type="button" className="btn-secondary px-3 py-2" disabled={filters.pageNo <= 1 || loading} onClick={() => { const next = { ...filters, pageNo: filters.pageNo - 1 }; setFilters(next); void loadData(next) }}>上一页</button><button type="button" className="btn-secondary px-3 py-2" disabled={filters.pageNo * filters.pageSize >= total || loading} onClick={() => { const next = { ...filters, pageNo: filters.pageNo + 1 }; setFilters(next); void loadData(next) }}>下一页</button></div></div>
          </AsyncState>
        </PageSection>
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/35 p-4 backdrop-blur-sm">
          <div className="w-full max-w-3xl rounded-3xl border border-slate-200 bg-white p-5 shadow-2xl sm:p-6">
            <div className="flex items-center justify-between gap-4">
              <div><div className="text-lg font-semibold text-slate-950">审计明细</div><div className="mt-1 text-sm text-slate-500">展示 detailJson 快照，便于定位业务上下文。</div></div>
              <button type="button" className="btn-secondary min-h-10 px-3 py-2" onClick={() => setSelectedLog(null)}><X className="h-4 w-4" /></button>
            </div>
            <div className="mt-4 space-y-4">
              <div className="glass-soft rounded-[24px] p-5 text-sm"><div className="flex items-center gap-2"><ShieldCheck className="h-4 w-4 text-primary-700" /><span className="font-semibold text-slate-900">基础信息</span></div><dl className="mt-4 space-y-3"><div className="flex items-center justify-between gap-3"><dt className="text-slate-500">审计 ID</dt><dd className="font-medium text-slate-900">{selectedLog.id}</dd></div><div className="flex items-center justify-between gap-3"><dt className="text-slate-500">业务类型</dt><dd><StatusBadge value={selectedLog.bizType} /></dd></div><div className="flex items-center justify-between gap-3"><dt className="text-slate-500">业务主键</dt><dd className="font-medium text-slate-900">{selectedLog.bizId}</dd></div><div className="flex items-center justify-between gap-3"><dt className="text-slate-500">操作动作</dt><dd><StatusBadge value={selectedLog.action} /></dd></div><div className="flex items-center justify-between gap-3"><dt className="text-slate-500">操作人</dt><dd className="font-medium text-slate-900">{selectedLog.operatorId ?? '--'}</dd></div><div className="flex items-center justify-between gap-3"><dt className="text-slate-500">来源 IP</dt><dd className="font-medium text-slate-900">{selectedLog.ip || '--'}</dd></div><div className="flex items-center justify-between gap-3"><dt className="text-slate-500">User-Agent</dt><dd className="max-w-[260px] truncate font-medium text-slate-900" title={selectedLog.userAgent || '--'}>{selectedLog.userAgent || '--'}</dd></div><div className="flex items-center justify-between gap-3"><dt className="text-slate-500">创建时间</dt><dd className="font-medium text-slate-900">{formatDateTime(selectedLog.createdAt)}</dd></div></dl></div>
              <div className="rounded-[24px] border border-slate-200/80 bg-white/75 p-5 backdrop-blur-xl"><div className="text-sm font-semibold text-slate-900">detailJson</div><pre className="mt-4 overflow-x-auto rounded-2xl bg-slate-950/95 p-4 text-xs leading-6 text-slate-100">{formattedDetailJson}</pre></div>
            </div>
          </div>
        </div>
      ) : null}
